...
export default function Inbox() {
  ...
  useEffect(() => {
    const channel = supabase
      .channel('inbound-messages-changes')
      .on(
        'postgres_changes',
        {
          event: 'INSERT',
      ...
      .on(
        ...
        (payload) => {
          console.log('Inbound message updated:', payload);
          queryClient.invalidateQueries(['inboundMessages']);
        }
      )
      .subscribe((status) => {
  ...
  const markAsReadMutation = useMutation({
    mutationFn: async (id) => {
      await fetch(`/api/messages/inbound/${id}`, {
        method: 'PATCH',
          ...
          <CardHeader>
            <div className="flex items-center justify-between">
              <CardTitle className="flex items-center gap-2">
                ...
                {isConnected ? (
                  <div className="flex items-center gap-1 text-xs text-green-600" title="Real-time connected">
                    <Wifi className="w-3 h-3" />
                    <span className="hidden sm:inline">Live</span>
                  </div>
                ) : (
                  <div className="flex items-center gap-1 text-xs text-gray-400" title="Real-time disconnected">
