...
### 4. **API Route: `messages/inbound/index.js`**
**Changes:**
- ✅ **Auto-creates contacts** for unknown SMS senders
- ✅ Checks if contact exists before creating (prevents duplicates)
- ✅ New contacts get `name: "Unknown"` by default
- ✅ Contact creation failure doesn't block message storage

**Behavior:**
When Twilio webhook receives SMS from a new number:
1. Finds matching chatroom by Twilio number
2. Checks if contact exists (phone + chatroom)
3. **If not found:** Auto-creates contact with `name: "Unknown"`
4. Stores message in `inbound_messages` and `messages` tables

**Example Flow:**
```
SMS arrives: From +15559999999 → To +15551234567
↓
1. Match chatroom (twilio_number = +15551234567)
2. Check contact exists
3. NOT FOUND → Create contact:
   {
     phone_number: "+15559999999",
     name: "Unknown",
     chatroom_id: "matched-chatroom-id"
   }
4. Store message
```
